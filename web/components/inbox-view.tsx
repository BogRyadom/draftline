...
export function InboxView({
  ...
  const accountsQuery = useQuery({
    queryKey: ["accounts"],
    queryFn: () => apiJson<Account[]>("/accounts"),
  });

  return (
    <div className="flex flex-col gap-6">
      <ConnectBanner gmail={gmail} reason={reason} />

      {accountsQuery.isError ? (
        <ErrorCard message={(accountsQuery.error as Error).message} />
      ) : accountsQuery.isPending ? (
        <Skeleton className="h-40 w-full" />
      ) : accountsQuery.data.length === 0 ? (
        <EmptyConnect />
      ) : (
        <ConnectedInbox account={accountsQuery.data[0]} filters={filters} />
      )}
    </div>
  );
}
...
function ConnectedInbox({
  ...
  const emailsQuery = useQuery({
    queryKey: ["emails", filters.category ?? null, filters.priority ?? null, filters.sort ?? null],
    queryFn: () => apiJson<EmailItem[]>(`/emails${qs ? `?${qs}` : ""}`),
    // Poll while newly-synced emails are still being classified in the background.
    refetchInterval: (q) => {
      const data = q.state.data as EmailItem[] | undefined;
      return data?.some((e) => e.status === "new" && !e.category) ? 4000 : false;
    },
  });
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {emailsQuery.isError ? (
        <ErrorCard message={(emailsQuery.error as Error).message} />
